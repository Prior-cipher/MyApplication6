...
wsServer.on("request", request => {
    ...
    connection.on("message", message => 
    {
        ...
        if (result.method === "surender1") {
            ...
            let k;
            for (const [key, value] of Object.entries(currentGame1)) {
                if (value.id1 == result.id) {
                    //sendwin
                    value.worker.terminate();
                    clients[value.id2].send(JSON.stringify({ "method": "surender1" }));
                    k = key;
                }
                else if (value.id2 == result.id) {
                    value.worker.terminate();
                    clients[value.id1].send(JSON.stringify({ "method": "surender1" }));
                    k = key;
                }
            }
                delete currentGame1[k];
